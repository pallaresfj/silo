...
class OidcClient
{
    public function buildAuthorizationUrl(
        ...
        $prompt = trim((string) ($prompt ?? $this->ssoConfig('prompt', 'login')));

        if ($prompt !== '') {
            $params['prompt'] = $prompt;
        }

        return $this->authorizationEndpoint().'?'.http_build_query($params, '', '&', PHP_QUERY_RFC3986);
    }
    ...
    public function validateIdToken(string $idToken): array
    {
        ...
        if (! $this->audienceContainsClientId($claims['aud'] ?? null)) {
    ...
    public function resolveClaims(array $tokens, array $idTokenClaims): array
    {
        ...
        if ($accessToken !== '' && $this->missingIdentityClaims($claims)) {
            foreach ($this->userinfo($accessToken) as $key => $value) {
    ...
    {
        $cacheKey = 'agroista-core.discovery.'.sha1($this->discoveryUrl());
    ...
    private function jwks(): array
    {
        ...
        $cacheKey = 'agroista-core.jwks.'.sha1($jwksUrl);
    ...
    {
        return (string) $this->ssoConfig('discovery_url', '');
    }

    private function issuer(): string
    {
        return (string) $this->ssoConfig('issuer', '');
    }

    private function clientId(): string
    {
        return (string) $this->ssoConfig('client_id', '');
    }

    private function clientSecret(): string
    {
        return (string) $this->ssoConfig('client_secret', '');
    }

    private function redirectUri(): string
    {
        return (string) $this->ssoConfig('redirect_uri', '');
    }
    ...
    private function scopes(): array
    {
        ...
        $scopes = $this->ssoConfig('scopes', ['openid', 'email', 'profile']);
    ...
    {
        return (int) $this->ssoConfig('http_timeout', 10);
    }

    private function discoveryCacheSeconds(): int
    {
        return (int) $this->ssoConfig('discovery_cache_seconds', 3600);
    }

    private function jwksCacheSeconds(): int
    {
        return (int) $this->ssoConfig('jwks_cache_seconds', 3600);
    }

    private function ssoConfig(string $key, mixed $default): mixed
    {
        $value = config('agroista-core.sso.'.$key);

        if ($value === null) {
            $value = config('sso.'.$key);
        }
